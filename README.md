...
<p>In this code, I run an experiment 10,000 times with 500 trials, i.e., after sampling from both variants a total of 500 times, which variant has the larger conversion rate?  After doing this 10,000 times, how often does the (theoretically) better variant come out on top?  I repeated this process experiments with 600, 700, 800, 900, and 1000 trials.  As expected, the better variant comes out on top a larger proportion of the time as the number of trials increases (see the table below).  I then compare this with a power analysis to determine the sample size needed for a variety of p-values, i.e, p=.2, .1, .05, and .01.  It is important to remember that this is simply the sample needed to have a valid A/B test in the traditional framework.  
